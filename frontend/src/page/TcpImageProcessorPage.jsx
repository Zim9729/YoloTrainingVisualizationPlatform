    ...
    const loadFolderFiles = async () => {
        ...
        try {
            ...
            if (response && response.code === 200 && response.data) {
                ...
                alert(`递归扫描完成！\n找到 ${response.data.total_files} 个图像文件\n分布在 ${dirCount} 个目录中\n总大小: ${response.data.total_size_mb} MB`);
            } else {
                alert('获取文件列表失败: ' + (response?.msg || '未知错误'));
                setFolderFiles([]);
            }
        } catch (error) {
    ...
    const processFolderImages = async () => {
        ...
        try {
            ...
            if (response && response.code === 200 && response.data) {
                ...
                alert('批量处理失败: ' + (response?.msg || '未知错误'));
            }
        } catch (error) {
