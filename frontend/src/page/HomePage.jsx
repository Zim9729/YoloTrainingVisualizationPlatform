                ...
                <div className="card-group" style={{ marginTop: '20px' }}>
                    <div className="card b-transform">
                        ...
                        <div style={{ display: 'flex', gap: '8px', marginTop: '12px' }}>
                            <button className="btn sm" onClick={() => { navigate("/dataset?type=uploadDataset") }}>上传压缩包</button>
                            <button className="btn sm" onClick={() => { navigate("/import/label-studio") }}>从 Label Studio 导入</button>
                        </div>
                    </div>
                    <div className="card b-transform pointer" onClick={() => { navigate("/tasks?type=newTask") }}>
                    ...
                    <div className="card b-transform pointer" onClick={() => { navigate("/tasks") }}>
                        <div className="top-tag">3</div>
                        <h1 className="title">启动模型训练</h1>
                        <p className="content">点击"开始训练"，系统将在后台自动调度任务，实时跟踪训练进度。</p>
                    </div>
                    <div className="card b-transform pointer" onClick={() => { navigate("/models?type=trained") }}>
                    ...
                    {runningTasks.length <= 0 ? (
                        ...
                        <div className="list-card-group">
                            {runningTasks.map((item, index) => (
                                <div key={`runningTaskList_${index}`} className="list-card" style={{ wordBreak: 'break-all' }} onClick={() => navigate(`/tasks/${encodeURIComponent(item.filename)}`)}>
